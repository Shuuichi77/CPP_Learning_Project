...
public:
    ...
    void start_service(const Aircraft& aircraft)
    {
        ...
        std::cout << "now servicing " << aircraft.get_flight_num() << " (fuel = " <<
                  current_aircraft->get_fuel() << ")...\n";
        service_progress = 0;
    }
    ...
    void finish_service()
    {
        if (!is_servicing())
        {
            std::cout << "done servicing " << current_aircraft->get_flight_num() << " (fuel = " <<
                      current_aircraft->get_fuel() << ")\n";
    ...
    {
        if (in_use() && is_servicing() && current_aircraft->is_low_on_fuel() && current_aircraft->is_on_ground())
        {
