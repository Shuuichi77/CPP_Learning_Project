...
public:
    AirportType(const Point3D& crossing_pos_, const Point3D& gateway_pos_,
            ...
            runways { runways_ }
    {
        assert(!terminal_pos.empty());
        assert(!runways.empty());
    }

    size_t get_nb_terminals() const { return terminal_pos.size(); }
                                  ...
                                  const size_t terminal_num) const
    {
        ...
        const Waypoint before_in_air { offset + runway.start - runway_length + Point3D { 0.f, 0.f, .7f },
                                       wp_air };
